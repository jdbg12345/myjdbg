...
export const OrganizationalStructure = () => {
  ...
  const departmentData = {
    ...
    '内控部': { 
      TH: { current: 6, planned: 0, isMain: false }, 
      DB: { current: 2, planned: 15, isMain: false }, 
      SLK: { current: 0, planned: 15, isMain: true }, 
      KJY: { current: 5, planned: 15, isMain: false }, 
      '孵化部': { current: 0, planned: 0, isMain: false }, 
      '外包': { current: 0, planned: 0, isMain: false }, 
      manager: '内控经理1人' 
    },
...
KJY为主要招聘地区，三季度目标扩张人员至30～50人；TH人员主迁移区，后续为主要招聘地区；DB后续视场地数量进行后续人员规
      ...
      {/* 核心交叉表格：纵向场地，横向部门 - 添加红点标识和图例 */}
      <div className="bg-white rounded-xl p-8 mb-8 border border-gray-200">
        <div className="flex items-center space-x-4 mb-8">
          <div className="w-12 h-12 bg-gray-700 rounded-full flex items-center justify-center">
            <Users className="w-7 h-7 text-white" />
          </div>
          <h2 className="text-3xl font-bold text-gray-800">部门分布统计</h2>
          {/* 图例说明 - 添加红点说明 */}
          <div className="flex items-center space-x-4 bg-white rounded-lg p-3 border border-blue-200 ml-8">
            <Info className="w-5 h-5 text-blue-600" />
            <div className="flex items-center space-x-3">
              <div className="bg-blue-500 text-white px-3 py-1 rounded-md text-sm font-semibold">-</div>
              <span className="text-sm text-gray-600">表示当前人数</span>
            </div>
            <div className="w-px h-6 bg-gray-300"></div>
            <div className="flex items-center space-x-3">
              <div className="bg-green-500 text-white px-3 py-1 rounded-md text-xs font-medium">-</div>
              <span className="text-sm text-gray-600">表示计划招聘</span>
            </div>
            <div className="w-px h-6 bg-gray-300"></div>
            <div className="flex items-center space-x-3">
              <div className="w-2 h-2 bg-red-500 rounded-full"></div>
              <span className="text-sm text-gray-600">表示未来主要场地</span>
            </div>
          </div>
        </div>
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-gray-700 text-white">
                <th className="p-3 text-left font-bold text-sm border-r border-gray-600 w-24">场地 \ 部门</th>
                <th className="p-3 text-center font-bold text-sm bg-blue-600 w-16 border-r border-gray-600">合计</th>
                {Object.keys(departmentData).map((dept, index) => (
                  <th key={index} className="p-2 text-center font-bold text-xs border-r border-gray-600 w-20">
                    <div className="mb-1">{dept}</div>
                    <div className="text-xs text-gray-300">{departmentData[dept].manager}</div>
                  </th>
                ))}
              </tr>
            </thead>
            <tbody>
              ...
              <tr className="border-b border-gray-200 hover:bg-white transition-colors">
                <td className="p-3 font-bold text-base text-gray-700 bg-white border-r border-gray-200">
                ...
                <td className="p-3 text-center bg-blue-100 font-bold text-lg text-blue-700 border-r border-gray-200">
                  {locationTotals.TH}
                </td>
                {Object.keys(departmentData).map((dept, index) => (
                  <td key={index} className="p-2 text-center border-r border-gray-200">
                    {renderPersonnelData(
                      departmentData[dept].TH.current, 
                      departmentData[dept].TH.planned, 
                      'bg-blue-500',
                      departmentData[dept].TH.isMain
                    )}
                  </td>
                ))}
              </tr>
              ...
              <tr className="border-b border-gray-200 hover:bg-white transition-colors">
                ...
                {Object.keys(departmentData).map((dept, index) => (
                  <td key={index} className="p-2 text-center border-r border-gray-200">
                    {renderPersonnelData(
                      departmentData[dept].DB.current, 
                      departmentData[dept].DB.planned, 
                      'bg-blue-500',
                      departmentData[dept].DB.isMain
                    )}
                  </td>
                ))}
              </tr>
              ...
              <tr className="border-b border-gray-200 hover:bg-white transition-colors">
                ...
                {Object.keys(departmentData).map((dept, index) => (
                  <td key={index} className="p-2 text-center border-r border-gray-200">
                    {renderPersonnelData(
                      departmentData[dept].SLK.current, 
                      departmentData[dept].SLK.planned, 
                      'bg-blue-500',
                      departmentData[dept].SLK.isMain
                    )}
                  </td>
                ))}
              </tr>
              ...
              <tr className="border-b border-gray-200 hover:bg-white transition-colors">
                ...
                {Object.keys(departmentData).map((dept, index) => (
                  <td key={index} className="p-2 text-center border-r border-gray-200">
                    {renderPersonnelData(
                      departmentData[dept].KJY.current, 
                      departmentData[dept].KJY.planned, 
                      'bg-blue-500',
                      departmentData[dept].KJY.isMain
                    )}
                  </td>
                ))}
              </tr>
              ...
              <tr className="border-b border-gray-200 hover:bg-white transition-colors">
                ...
                {Object.keys(departmentData).map((dept, index) => (
                  <td key={index} className="p-2 text-center border-r border-gray-200">
                    {renderPersonnelData(
                      departmentData[dept]['孵化部'].current, 
                      departmentData[dept]['孵化部'].planned, 
                      'bg-blue-500',
                      departmentData[dept]['孵化部'].isMain
                    )}
                  </td>
                ))}
              </tr>
              ...
              <tr className="border-b border-gray-200 hover:bg-white transition-colors">
                ...
                {Object.keys(departmentData).map((dept, index) => (
                  <td key={index} className="p-2 text-center border-r border-gray-200">
                    {renderPersonnelData(
                      departmentData[dept]['外包'].current, 
                      departmentData[dept]['外包'].planned, 
                      'bg-blue-500',
                      departmentData[dept]['外包'].isMain
                    )}
                  </td>
                ))}
              </tr>
              ...
              <tr className="bg-blue-100 border-t-2 border-blue-300">
                <td className="p-3 font-bold text-base text-blue-700 bg-blue-200 border-r border-blue-300">
                  <div className="flex items-center space-x-2">
                ...
                <td className="p-3 text-center bg-blue-200 font-bold text-xl text-blue-800 border-r border-blue-300">
                  {Object.values(locationTotals).reduce((sum, count) => sum + count, 0)}
                </td>
                {Object.keys(departmentData).map((dept, index) => {
                  const deptTotal = Object.values(departmentData[dept])
                    .filter(location => typeof location === 'object' && location.current !== undefined)
                    .reduce((sum, location) => sum + location.current, 0);
                  
                  return (
                    <td key={index} className="p-2 text-center border-r border-blue-300">
                      <div className="bg-blue-600 text-white px-3 py-1 rounded-md text-sm font-semibold mx-auto w-fit">
                        {deptTotal}
                      </div>
                    </td>
                  );
