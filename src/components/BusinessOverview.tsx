...
export const BusinessOverview = () => {
  ...
  // 体系变化对比数据
  const systemChanges = [
    { direction: '人员的风控能力参差不齐', pain: '依赖个人经验与小圈子文化，资深人员不愿传授，大部分新人难以提升技能；审核存在时效考核，导致检查不全面，出现遗漏。', before: '按照金额派单；单人即可出款', now: '将核心复杂经验工具化和系统化，使所有人即便缺乏丰富经验，也能通过工具一键获取结果，从而提升审核人员整体能力下限。', example: '云盾分数、内控抽检' },
    { direction: '人的审核不按照标准执行', pain: '审核SOP缺乏有效监督和约束，导致关键细节（如标签备注、系统提醒等）经常被遗漏，引发出款错误，给公司造成损失。', before: '以防御型策略为主（新手类、金额类），缺少套利识别策略', now: '系统负责自动识别与拦截；流程负责在关键环节嵌入约束，减少人为失误空间；', example: '电子卡免费、代理关联、棋牌套利、彩票套利等' },
    { direction: '内外的人员进行勾结串联', pain: '风控策略、参数、标签及备注可能被泄露，甚至出现问题会员被直接放行的情况率', before: '一些核心策略和参数外界已"知晓"', now: '敏感信息至最小范围；参数动态化，表面可见，但实际执行不同；对敏感操作进行预警，实现综合管控，降低风险。逐步实施参数分层、动态设置', example: '部分***参数已经试运行中' },
