...
export const BusinessOverview = () => {
  ...
  const systemChanges = [
    ...
    { direction: '系统审核策略严重缺失', pain: '风控策略、参数、标签及备注可能被泄露，甚至出现问题会员被直接放行的情况率', before: '敏感信息至最小范围；参数动态化，表面可见，但实际执行不同；对敏感操作进行预警，实现综合管控，降低风险。逐步实施参数分层、动态设置', now: '敏感信息至最小范围；参数动态化，表面可见，但实际执行不同；对敏感操作进行预警，实现综合管控，降低风险。逐步实施参数分层、动态设置', example: '部分***参数已经试运行中' },
